 ...
 *
 * CLI:
 *   php sync.php --mode=fast   (по умолчанию) — dashboard, task_breakdown (4 статуса)
 *   php sync.php --mode=full   — fast + shifts, tasks, house_breakdown
 *
 * HTTP (cron):
...
$TASK_BREAKDOWN_STATUSES = ['planned', 'in_progress', 'missed', 'done'];

$FAST_ENDPOINTS = [
    ['dashboard', 'date=today'],
];
foreach ($TASK_BREAKDOWN_STATUSES as $st) {
    $FAST_ENDPOINTS[] = ['task_breakdown', 'date=today&status=' . $st];
}
